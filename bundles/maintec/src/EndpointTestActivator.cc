...
class EndpointTestActivator {
public:
    explicit EndpointTestActivator(const std::shared_ptr<celix::BundleContext>& ctx) :
        processorConsumer{ProcessorConsumer::create()}, tracker{createTracker(ctx)} {
        std::cout << "Started Endpoint Test from bundle with id " << ctx->getBundleId() << std::endl;
        auto reg = ctx->registerService<celix::IShellCommand>(processorConsumer)
                .addProperty(celix::IShellCommand::COMMAND_NAME, "endpoint")
                .build();
        regs.push_back(reg);
    }

    ~EndpointTestActivator() noexcept {
        std::cout << "Finished Endpoint Test" << std::endl;
    }
private:
...
CELIX_GEN_CXX_BUNDLE_ACTIVATOR(EndpointTestActivator)